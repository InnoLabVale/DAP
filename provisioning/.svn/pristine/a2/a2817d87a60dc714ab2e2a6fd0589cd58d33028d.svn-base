...
public class ProvisioningResponseGridManager implements BsoResponseGridManagmentInterface {
	...
	public void setResponseGridAssurance(String response_mpm){
		...
		if(response_mpm.equals(components.getErrorMessages().get_USER_NOT_FOUND()) ||
		   ...
		   response_mpm.equals(components.getErrorMessages().get_NOT_LOGGED_ERROR())){
			...
			if(!responsePlitted[2].equals("0")){
				responseGridField.add(new JLabel(readErrorDescription(responsePlitted[2])), components.getCommonGUI().setContraints(GridBagConstraints.VERTICAL, GridBagConstraints.WEST, 5, 20, 0, 0, 0, 0));
			}
		...
		responseField.validate();
		responseField.repaint();
	}
