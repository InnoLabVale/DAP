...
public class ActionLanguage implements ActionResourceInterface{
	...
	public String update(CommonResourceInterface resourceBean){
		...
		else if(resourceBean.equals(DAPBeanInterface.CORPORATE)){
			command.append(DAPBeanInterface.CH2);
		}
		command.append(AIRCommands.MSISDN).append(resourceBean.getMsisdn());
		command.append(AIRCommands.AIR_COMMAND_SET_LANGUAGE).append(resourceBean.getLanguage().getLangauge());
