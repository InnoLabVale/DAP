...
public class LanguageResponseGrid {

	private DAPBeanInterface components;
	
	public LanguageResponseGrid(DAPBeanInterface components){
	...
	public Map<Object, GridBagConstraints> getGrid(String[] mpm_response_splitted){
		...
		JLabel resource_label = new JLabel(resource_response_splitted[0]+":");
		...
		if(resource_label_value.getText().equals("1")){
			resource_label_value = new JLabel("ITALIANO");
		}
		
		else if(resource_label_value.getText().equals("2")){
			resource_label_value = new JLabel("ENGLISH");
		}
		
		else{
			resource_label_value = new JLabel("");
		}
